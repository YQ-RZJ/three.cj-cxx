...
#if BX_PLATFORM_LINUX
		// OHOS 等平台 EGL 库名不带版本后缀（libEGL.so），回退再试一次
		if (NULL == handle)
		{
			handle = bx::dlopen("libEGL.so");
		}
...
#if BX_PLATFORM_LINUX
		// OHOS 等平台 GLES 库名不带版本后缀（libGLESv2.so / libGLESv3.so），回退再试
		if (NULL == lib)
		{
			lib = bx::dlopen("libGLESv2.so");
		}
		if (NULL == lib)
		{
			lib = bx::dlopen("libGLESv3.so");
		}
#endif // BX_PLATFORM_LINUX
