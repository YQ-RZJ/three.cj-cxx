...
#  endif
    ...
    // OHOS musl 无 getlogin_r 的可靠实现差异，走 passwd gecos（对齐 Android 行为）
    const auto passwd = getpwuid( getuid() );
    if( passwd && passwd->pw_name && *passwd->pw_name ) return passwd->pw_name;
    return "(?)";
#else
...
#  endif
    ...
    return nullptr;
#elif defined __OHOS__
    // OHOS musl passwd 无 gecos，用 pw_name（对齐 Android 的降级行为）
    const auto passwd = getpwuid( getuid() );
    if( passwd && passwd->pw_name && *passwd->pw_name ) return passwd->pw_name;
    return nullptr;
#else
