...
// This is so far only needed on Android and OpenHarmony, where it is common for libraries
// to be mapped with only executable, not readable, permissions. Typical example (line from /proc/self/maps):
/*
...
void Profiler::HandleSymbolQueueItem( const SymbolQueueItem& si )
{
    ...
    {
#if defined __ANDROID__ || defined __OHOS__
        // On Android/OpenHarmony it's common for code to be in mappings that are
