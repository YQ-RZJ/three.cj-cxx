...
extern void CJ_TLS_DYN_DeleteSession(SSL_SESSION* session)
{
    if (session == NULL) {
        return;
    }
...
static SSL_SESSION* CopySession(SSL_SESSION* session)
{
    unsigned char* p = NULL;
    int size = i2d_SSL_SESSION(session, &p);
    if (size <= 0) {
        return 0;
    }

    const unsigned char* p2 = p;
    SSL_SESSION* copy = d2i_SSL_SESSION(NULL, &p2, (long)size);
    OPENSSL_free(p);
    ...
    unsigned int len = 0;
    const unsigned char* ctx = SSL_SESSION_get0_id_context(session, &len);
    if (ctx != NULL && len > 0) {
        SSL_SESSION_set1_id_context(copy, ctx, len);
    }

    return copy;
}

extern int CJ_TLS_DYN_SetSession(SSL* stream, SSL_SESSION* session)
{
    if (stream == NULL || session == NULL) {
        return 1;
    }

    if (SSL_SESSION_is_resumable(session) == 0) {
        SSL_SESSION* copy = CopySession(session);
        int result = CJ_TLS_DYN_SetSession(stream, copy);
        SSL_SESSION_free(copy);
        return result;
    }

    if (SSL_set_session(stream, session) == 0) {
        return 1;
    }
...
extern int CJ_TLS_DYN_AddSession(SSL_CTX* ctx, SSL_SESSION* session)
{
    ...
    if (SSL_SESSION_is_resumable(session) == 0) {
        SSL_SESSION* copy = CopySession(session);
        int result = CJ_TLS_DYN_AddSession(ctx, copy);
        SSL_SESSION_free(copy);
        return result;
    }

    if (SSL_CTX_add_session(ctx, session) == 0) {
        return 1;
    }
...
extern void CJ_TLS_DYN_IncrementUse(SSL_SESSION* session)
{
    if (session != NULL) {
...
int NewSessionCallback(SSL* ssl, SSL_SESSION* session) // (SSL *ssl, SSL_SESSION *session)
{
    ...
    const unsigned char* sessionId = SSL_SESSION_get_id(session, &idLength);
...
static SSL_SESSION* GetSessionCallback(SSL* ssl, const unsigned char* data, int len, int* copy)
{
    ...
    if (result != NULL) {
        ...
        SSL_SESSION* sessionCopy = CopySession(result);
        SSL_SESSION_free(result); // this should be done after doing a copy
        result = sessionCopy;
    }
...
extern int CJ_TLS_DYN_SetSessionIdContext(SSL_CTX* ctx, const unsigned char* sidCtx, unsigned int sidCtxLen)
{
    ...
    return SSL_CTX_set_session_id_context(ctx, sidCtx, sidCtxLen);
}

extern void CJ_TLS_DYN_GetSessionId(const SSL_SESSION* session, const unsigned char** data, size_t* length)
{
    ...
    const unsigned char* returnedData = SSL_SESSION_get_id(session, &returnedSize);
