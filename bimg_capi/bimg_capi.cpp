...
bool bimg_ic_get_ktx_le(const bimg_image_container_t* _ic) {
    return castICConst(_ic)->m_ktxLE;
}
...
bimg_image_container_t* bimg_image_parse_gnf(bimg_allocator_t* _allocator, const void* _src, uint32_t _size, bimg_error_t* _err) {
    bimg::ImageContainer* ic = bimg::imageParseGnf(castAlloc(_allocator), _src, _size, castErr(_err));
    return reinterpret_cast<bimg_image_container_t*>(ic);
}
