...
#define SL_IID_VOLUME (*pSL_IID_VOLUME)
#else
...
#ifdef __OHOS__
    void process(SLOHBufferQueueItf bq, void *pContext, SLuint32 size) noexcept;
#else
...
#ifdef __OHOS__
/* OHOS 缓冲队列是拉模式：系统请求数据时回调（size=请求字节数），
 * GetBuffer 拿系统空闲 buffer，OpenAL 混音填充后 Enqueue。
 * 参考 HarmonyOS OpenSL ES 文档的 BufferQueueCallback 模式。 */
void OpenSLPlayback::process(SLOHBufferQueueItf bq, void*, SLuint32 size) noexcept
{
    SLuint8 *buffer{nullptr};
    SLuint32 bufferSize{0};
    SLresult result{VCALL(bq,GetBuffer)(&buffer, &bufferSize)};
    if(SL_RESULT_SUCCESS == result && buffer != nullptr)
    {
        const auto frames = static_cast<uint>(size) / mFrameSize;
        std::unique_lock<std::mutex> dlock{mMutex};
        mDevice->renderSamples(buffer, frames, mDevice->channelsFromFmt());
        dlock.unlock();
        VCALL(bq,Enqueue)(buffer, size);
    }
}
#else
...
#ifdef __OHOS__
    const std::array<SLInterfaceID,1> ids{{ SL_IID_OH_BUFFERQUEUE }};
...
{
#ifdef __OHOS__
    /* OHOS 拉模式：注册回调后 SetPlayState(PLAYING)，系统自动按需请求数据
     *（回调触发 GetBuffer→混音→Enqueue），无需混音线程。 */
    SLOHBufferQueueItf bufferQueue;
    SLresult result{VCALL(mBufferQueueObj,GetInterface)(SL_IID_OH_BUFFERQUEUE,
        &bufferQueue)};
    ...
    if(SL_RESULT_SUCCESS == result)
    {
        result = VCALL(bufferQueue,RegisterCallback)(
            ...
            { static_cast<OpenSLPlayback*>(context)->process(bq, context, size); }, this);
    ...
    if(SL_RESULT_SUCCESS != result)
        throw al::backend_exception{al::backend_error::DeviceError,
            "Failed to start playback: {:#08x}", result};
#else
    mRing->reset();
...
void OpenSLPlayback::stop()
{
    ...
    SLOHBufferQueueItf bufferQueue;
    result = VCALL(mBufferQueueObj,GetInterface)(SL_IID_OH_BUFFERQUEUE, &bufferQueue);
    PrintErr(result, "bufferQueue->GetInterface");
    if(SL_RESULT_SUCCESS == result)
    {
