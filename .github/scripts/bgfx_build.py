...
def parse_args():
    ...
    ap.add_argument("--libs", default=None,
                    help="逗号分隔的库清单（按序）：bx,bimg,bimg_capi,bgfx,geometryc_capi,shaderc_capi（默认=全量编译）")
...
def build_one(platform, mode, arch, libtype, toolchain, host, args, ctx, log_path):
    # CI-PATCH: --libs 选择性构建（默认全量），只编指定子项目
    wanted = parse_libs_arg(args.libs, _ALLOWED_LIBS, "bgfx")
    for proj in SUBPROJECTS:
        if proj not in wanted:
            print("  [skip] %s（--libs 未包含）" % proj)
            continue
...
def main():
    global _BATCH_FLAG
    args = parse_args()
    libs_wanted = parse_libs_arg(args.libs, _ALLOWED_LIBS, "bgfx")
