...
def build_tlsbridge(platform, arch, mode, host, ctx, ssl_dir, log_path, libtype="static"):
    ...
    if platform == "WINDOWS":
        ...
        # 原先用宿主 "cc"，libtlsbridge.so 被编成 x86_64（归包审计实测）。
        # 用 aarch64-linux-gnu 三元组工具链（与 OpenSSL 的交叉注入同款）；
        # 找不到时报错而非悄悄产出异架构库。x86_64/BSD 保持宿主编译器。
        if platform == "LINUX" and arch == "arm64-v8a":
            cross_prefix = "aarch64-linux-gnu"
            cross_cc = (shutil.which(cross_prefix + "-gcc")
                        or shutil.which(cross_prefix + "-clang"))
            if not cross_cc:
                raise RuntimeError("缺少 aarch64-linux-gnu 交叉工具链"
                                   "（apt install gcc-aarch64-linux-gnu）")
            cc = cross_cc
            ar = cross_prefix + "-ar"
            # CI-PATCH2: --target 是 clang 语法，GNU gcc 报 unrecognized
            # command-line option（linux arm64 job 实测）——gcc 靠三元组
            # 前缀本身隐含目标，无需 --target；clang 才需要显式传。
            if cross_cc.endswith("-clang"):
