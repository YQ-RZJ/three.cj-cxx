...
def stage_imgui_deps(cxx_root: str, arch: str) -> str:
    ...
    def _lib_machine(path):
        ...
        if head[:4] in (b"\xcf\xfa\xed\xfe", b"\xfe\xed\xfa\xcf"):
            # CI-PATCH7: Mach-O thin——读 cputype 精确判架构。旧版统一返回
            # "macho" 放行，macos x86 job 实测 arm64-only 的 libSDL3.dylib
            # 残留混进 x86_64 暂存区（同 job 全量矩阵 + 跨轮留存），链接器
            # 取不到任何 x86_64 符号 → SDL 全部基础符号 undefined。
            # MH_MAGIC_64 小端 \xcf\xfa\xed\xfe / 大端 \xfe\xed\xfa\xcf，
            # cputype 在偏移 4（0x01000007=x86_64, 0x0100000C=arm64）
            ct = struct.unpack_from("<I", head, 4)[0]
            if head[:4] == b"\xfe\xed\xfa\xcf":  # 大端字节序文件
                ct = struct.unpack_from(">I", head, 4)[0]
            return {0x01000007: "x86_64", 0x0100000C: "arm64"}.get(ct)
        if head[:4] in (b"\xca\xfe\xba\xbe", b"\xbe\xba\xfe\xca"):
            # Mach-O fat（通用二进制）——遍历 fat arch 表查是否含目标架构
            # （fat_header: magic+nfat_arch；每项 20 字节，cputype 在项内
            # 偏移 0。cafebabe 大端）。含目标架构即放行。
            try:
                nfat = struct.unpack_from(">I", head, 4)[0]
                if head[:4] == b"\xbe\xba\xfe\xca":  # little-endian fat
                    nfat = struct.unpack_from("<I", head, 4)[0]
                    ct_off, ct_fmt, want_off = 8, "<I", 0
                else:
                    ct_off, ct_fmt, want_off = 8, ">I", 0
                with open(path, "rb") as fh:
                    data = fh.read(8 + nfat * 20)
                for i in range(nfat):
                    ct = struct.unpack_from(ct_fmt, data, 8 + i * 20)[0]
                    if ct in (0x01000007, 0x0100000C):
                        return {0x01000007: "x86_64",
                                0x0100000C: "arm64"}[ct]
                return None  # fat 里没有目标架构
    ...
    for src_root in ("output", "build"):
        ...
        for pattern in ("*.a", "*.lib", "*.so", "*.dylib"):
            for f in glob.glob(os.path.join(base, "**", pattern), recursive=True):
                ...
                # CI-PATCH6: 无条件覆盖——旧逻辑 os.path.isfile(dst) 跳过
                # 已存在文件，ci_deps/ 不在 --clean 范围内，跨轮存活的旧版
                # 依赖（上一轮 SDL3 dylib 缺 SDL_SetTextInputArea 等新 API，
                # macos x86_64 job 实测 imgui shared 链接 undefined；arm64
                # runner 无残留所以通过）会永久遮蔽本轮新产物。
                shutil.copy2(f, dst)
