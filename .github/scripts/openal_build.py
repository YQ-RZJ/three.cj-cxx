...
# -*- coding: utf-8 -*-
"""
openalsoft4cj 全平台自动交叉编译 + 打包脚本（openal-soft 版）
==============================================================
...
SRC_DIR    = os.path.join(SCRIPT_DIR, "openal-soft")   # openal-soft 源码目录
MA_DIR     = os.path.join(SCRIPT_DIR, "miniaudio")     # miniaudio 源码目录
...
# 各平台默认启用的后端（其余全部显式关闭，避免交叉编译时误探测宿主库）
BACKENDS_PLATFORM = {
...
ALL_BACKENDS = ["ALSA", "COREAUDIO", "DSOUND", "JACK", "OBOE", "OPENSL", "OSS",
                "PIPEWIRE", "PORTAUDIO", "PULSEAUDIO", "SDL2", "SDL3", "SNDIO",
                "SOLARIS", "WASAPI", "WAVE", "WINMM"]
...
def parse_args():
    ap = argparse.ArgumentParser(
        prog="build.py",
        description="openalsoft4cj 全平台自动交叉编译 + 打包脚本（openal-soft, CMake 构建）",
    ...
    ap.add_argument("--libtype", default=",".join(ALL_LIBTYPES),
                    help="库类型清单，逗号分隔: static,shared（默认全部，对应 CMake -DLIBTYPE）")
...
def probe_ndk_toolchain(ndk, host):
    ...
    openal-soft 官方 XCompile-Android.txt 要求使用 NDK 自带的
...
def probe_mingw(arch, mingw_dir):
    ...
    if arch == "arm64-v8a":
        ...
        # CI-PATCH2: 删除"Windows 主机本机 gcc 优先"分支——--mingw 指向
        # llvm-mingw(msvcrt) 时必须整体一致，x86_64 落回本机 mingw
        # (UCRT/libstdc++) 会混出两套 C++ 标准库（std::__1 vs __cxx11），
        # 消费侧 -lc++/-lstdc++ 单挂哪套都 undefined/重复定义
        # （win x86_64 job 实测混链）
    for pre in triples:
        # 1) --mingw 指定目录
...
def toolchains_for(platform, host, libtype):
    """每个平台候选工具链（按优先级）：WINDOWS 主机上静态库优先 mingw，其余 msvc 优先，失败回退另一工具链"""
...
def toolchain_cfg(platform, arch, toolchain, host, ctx, args):
    """生成平台/工具链专用 CMake 参数（openal-soft 与 miniaudio 共用）"""
    cfg = []
    if platform == "WINDOWS":
        if toolchain == "msvc":
        ...
        # 原先无 LINUX 分支，CMake 落到宿主原生 gcc，libopenal.so /
        # libminiaudio.so 被编成 x86_64（归包审计实测）。用
        # aarch64-linux-gnu 三元组工具链；找不到时报错而非悄悄产出异架构库。
        if arch == "arm64-v8a":
        ...
        # CI-PATCH2: Xcode 生成器默认对 iOS target 启用代码签名，CI 无
        # 开发者证书必然失败（ios arm64 job 实测：Signing for "miniaudio"
        # requires a development team）。构建的是不被 Xcode 消费的裸
        # 静态/动态库，签名无意义——构建期与工程属性双层禁用。
        cfg += [
...
def miniaudio_config(platform, mode, arch, libtype, toolchain, host, ctx, args):
    ...
    miniaudio 只需解码能力（ma_decode_file），因此显式关闭全部设备后端，
    避免交叉编译时误探测宿主库。
    """
    ...
    cfg += [
        ...
        # 关闭辅助节点库/外部解码器（MA_NO_NODE_GRAPH 下无法编译）
        "-DMINIAUDIO_NO_EXTRA_NODES=ON",
        "-DMINIAUDIO_NO_LIBVORBIS=ON",
        "-DMINIAUDIO_NO_LIBOPUS=ON",
        # MA_NO_DEVICE_IO：禁用播放/录制（ma_context/ma_device），
        # MA_NO_ENGINE / MA_NO_NODE_GRAPH：禁用 3D 引擎（ma_engine/ma_sound），
        # MA_NO_RESOURCE_MANAGER：禁用资源管理器，
        # MA_NO_GENERATION：禁用波形/噪声生成器，
        # MA_NO_THREADING：禁用线程 API。
        # 综合将 miniaudio 裁剪为纯解码库（ma_decode_*/ma_decoder_* 保留），
        # 播放统一由 OpenAL 负责。
        "-DCMAKE_C_FLAGS=-DMA_NO_DEVICE_IO -DMA_NO_ENGINE -DMA_NO_NODE_GRAPH -DMA_NO_RESOURCE_MANAGER -DMA_NO_GENERATION -DMA_NO_THREADING",
    ]
    # miniaudio 的 CMake 选项：MINIAUDIO_NO_XXX=ON -> MA_NO_XXX 编译宏
    for b in ["WASAPI", "DSOUND", "WINMM", "ALSA", "PULSEAUDIO", "JACK",
              "COREAUDIO", "SNDIO", "AUDIO4", "OSS", "AAUDIO", "OPENSL",
              "WEBAUDIO", "CUSTOM", "NULL"]:
        cfg.append("-DMINIAUDIO_NO_%s=ON" % b)
...
def cmake_config(platform, mode, arch, libtype, toolchain, host, ctx, args):
    ...
    cfg += [
        ...
        "-DALSOFT_DLOPEN=OFF",
        "-DALSOFT_WERROR=OFF",
        "-DLIBTYPE=" + libtype.upper(),
    ]
    # MSVC 上 openal-soft 的 C++20 modules 在部分编译器版本存在
    # "找不到模块 alc.context" 兼容问题，关闭后回退传统头文件方式
    if platform == "WINDOWS" and toolchain == "msvc":
        cfg.append("-DALSOFT_ENABLE_MODULES=OFF")
    # 显式关闭所有后端，再按平台打开需要的
    for b in ALL_BACKENDS:
        cfg.append("-DALSOFT_BACKEND_%s=OFF" % b)
    for b in BACKENDS_PLATFORM.get(platform, ["WAVE"]):
        cfg.append("-DALSOFT_BACKEND_%s=ON" % b)

    # ---- 平台 / 工具链专用参数（与 miniaudio 共用）----
    cfg += toolchain_cfg(platform, arch, toolchain, host, ctx, args)
    return cfg, is_multi
...
def build_env(platform, emsdk):
    ...
    if platform == "OHOS":
        ...
        # __OHOS__，但临时 toolchain 回退方案（_gen_ohos_toolchain）不保证，
        # 此处显式注入宏以确保 opensl.cpp 的 OHOS 定制分支必定生效。
        # 注意用环境变量 CFLAGS/CXXFLAGS 而非 -DCMAKE_C_FLAGS：miniaudio 的
        # miniaudio_config() 已用 -DCMAKE_C_FLAGS 覆盖式传参，两者互不干扰。
        env["CFLAGS"] = (env.get("CFLAGS", "") + " -D__OHOS__").strip()
        env["CXXFLAGS"] = (env.get("CXXFLAGS", "") + " -D__OHOS__").strip()
    return env
...
def build_one(platform, mode, arch, libtype, toolchain, host, args, ctx, log_path):
    ...
    print("  [%s/%s/%s] 编译 miniaudio ..." % (mode, arch, libtype))
    ma_build_cmd = [cmake, "--build", ma_build_dir, "--parallel", str(args.jobs)]
    if ma_multi:
        ma_build_cmd += ["--config", mode.capitalize()]
    if run(ma_build_cmd, SCRIPT_DIR, env, log_path) != 0:
        return False

    print("  [%s/%s/%s] 安装 miniaudio 到 %s ..." % (mode, arch, libtype, stage_dir))
    ma_install_cmd = [cmake, "--install", ma_build_dir, "--prefix", stage_dir]
    if ma_multi:
        ma_install_cmd += ["--config", mode.capitalize()]
    if run(ma_install_cmd, SCRIPT_DIR, env, log_path) != 0:
        return False
    return True
...
def package(platform, mode, arch, libtype, toolchain, dist_dir, args):
    name = "openalsoft4cj-%s-%s-%s-%s-%s" % (platform.lower(), arch, mode, libtype, toolchain)
...
def main():
    ...
    host = detect_host()
    print("=" * 60)
    print("  openalsoft4cj 全平台交叉编译 + 打包（openal-soft）")
    print("=" * 60)
        ...
        for mode in modes:
            for arch in arches:
                for libtype in libtypes:
                    combo_name = "openalsoft4cj-%s-%s-%s-%s" % (platform.lower(), arch, mode, libtype)
