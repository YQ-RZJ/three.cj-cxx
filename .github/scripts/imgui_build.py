...
def parse_args():
    ...
    ap.add_argument("--libtype", default="static,shared",
                    help="库类型清单，逗号分隔: static,shared（CI-PATCH: 默认双模式"
                         "——CI 未传 --libtypes 时 imgui shared 全平台缺失实测；"
                         "shared 需链接 SDL3/bgfx 预编译产物，缺依赖时自动跳过"
                         " shared 组合而非报错）")
...
def probe_mingw(arch, mingw_dir):
    ...
    # （win x86_64 job 实测 imgui/jolt/openal/requireCJLib 混链）。
    search_dirs = []
    if mingw_dir:
    ...
    if libtype == "shared":
        ...
        for d in deps_dirs:
            ...
            for f in sorted(os.listdir(d)):
                lf = f.lower()
                # CI-PATCH2: 只收动态库与导入库——--deps-lib 目录里
                # static+shared 产物共存（ci_deps 同时暂存 libSDL3.a 与
                # .so），静态 .a 非 PIC，链进 libimgui.so 报 "relocation
                # R_X86_64_PC32 ... recompile with -fPIC"（linux job 实测）。
                # CI-PATCH3（Windows）: .lib 需区分导入库与真静态库——
                # "SDL3-static.lib" 是 MSVC 静态归档，内嵌 /DEFAULTLIB
                # MSVCRTD 指令，llvm-mingw 的 lld 链接时报 "could not open
                # 'libMSVCRTD.a'"（windows arm64 job 实测）——文件名含
                # "-static" 的排除；MinGW 导入库 .dll.a 必须收（bgfx 组
                # 在 windows 上由 mingw 构建，动态链接依赖即 .dll.a）。
                if lf.endswith((".so", ".dylib")):
                    dep_libs.append(os.path.join(d, f).replace("\\", "/"))
                elif platform == "WINDOWS" and lf.endswith(".dll.a"):
                    dep_libs.append(os.path.join(d, f).replace("\\", "/"))
                elif platform == "WINDOWS" and lf.endswith(".lib") and "-static" not in lf:
                    dep_libs.append(os.path.join(d, f).replace("\\", "/"))
