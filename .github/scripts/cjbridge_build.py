...
def toolchain_cfg(platform, arch, toolchain, host, ctx, args):
    ...
    if platform == "WINDOWS":
        ...
        # 原先无 LINUX 分支，CMake 落到宿主原生 gcc，librequirecj_ffi.so /
        # libdlbridge.so 被编成 x86_64（归包审计实测）。用
...
def build_one(platform, mode, arch, libtype, toolchain, host, args, ctx, log_path):
    ...
    #    CI-PATCH: 非 OHOS 平台显式打印跳过原因——此前静默不编，产物核对
    #    时 librequirecj_napi "无产物" 难以判别是缺失还是设计如此（实测）
    if not args.skip_napi and platform == "OHOS":
        if "napi" not in wanted:
            ...
            if not build_subproject(NAPI_SRC_DIR, napi_build, napi_stage,
                ...
                return False
    elif platform != "OHOS" and "napi" in wanted:
        print("  [skip] requireCJLib-ark (NAPI)（%s 平台无 node_api.h，仅 OHOS 编译）" % platform)
