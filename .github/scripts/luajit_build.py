...
def build_config(platform, mode, arch, libtype, toolchain, host, ctx):
    ...
    # CI-PATCH2: Apple 平台（OSX/IOS）动态库惯例命名是 .dylib，但 LuaJIT
    # Makefile 只有 libluajit.so 的规则——TARGET_T 覆盖成 .dylib 会报
    # "No rule to make target 'libluajit.dylib'"（ios/osx job 实测）。
    # 故构建仍用默认 TARGET_T=libluajit.so（顺带跳过裸可执行链接），
    # 打包阶段再把 .so 重命名为 .dylib（见 package()）。
    ...
    if platform in ("ANDROID", "OHOS"):
        ...
        vars.append("TARGET_LD=" + target_cc)
        # CI-PATCH2 的 TARGET_T=libluajit.dylib 已提升至平台层统一覆盖
        # （见 vars 构造后），此处不再重复
    elif platform == "IOS":
        # CI-PATCH: Makefile 默认 TARGET_LD=$(CROSS)$(CC)=宿主 gcc——
        ...
        # 裸可执行文件。FFI 消费的只是动态库，覆盖 TARGET_T 为仅动态库，
        # 跳过可执行链接（Apple 命名 .dylib 由打包阶段重命名完成，见
        # package()；Makefile 无 libluajit.dylib 规则，不能直接覆盖）。
        vars.append("TARGET_T=libluajit.so")
    if mode == "debug":
...
def package(platform, mode, arch, libtype, toolchain, dist_dir):
    ...
    for f in os.listdir(SRC_DIR):
        ...
        if os.path.isfile(full) and f.endswith(LIB_EXTENSIONS):
            # CI-PATCH2: 按 libtype 过滤——就地构建目录里可能同时残留
            # .a 与 .so/.dylib（跨组合构建未清理），全收会把动态库打进
            # static 包（归包审计实测 iOS static 包混入 libluajit.so）。
            low = f.lower()
            if libtype == "static" and not low.endswith(".a"):
                continue
            if libtype == "shared" and not low.endswith((".so", ".dylib", ".dll")):
                continue
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        # 库文件。CI-PATCH3: Apple 平台（OSX/IOS）的动态库惯例命名是
        # .dylib——构建仍产出 libluajit.so（Makefile 只有该规则），打包
        # 时重命名为 .dylib（与 package 阶段 filtered 集合同步，见上）
        for lf in lib_files:
            arc = os.path.basename(lf)
            if platform in ("OSX", "IOS") and arc.lower() == "libluajit.so":
                arc = "libluajit.dylib"
            z.write(lf, os.path.join(name, "lib", arc))
