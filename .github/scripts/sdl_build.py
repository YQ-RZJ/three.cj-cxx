...
def cmake_configure_args(platform, mode, arch, libtype, toolchain, host, ctx):
    ...
    # ---- WINDOWS: 本机 mingw (clang/gcc) 或 msvc ----
        ...
        args += ["-DCMAKE_C_COMPILER=" + cc.replace("\\", "/"),
        ...
        if rc:
            # CI-PATCH2: CMake 工具链路径统一正斜杠——反斜杠路径写入
            # CMakeRCCompiler.cmake 时 `\l` 等被当非法转义序列，配置直接
            # 失败（"Invalid character escape '\l'"，win x86_64 job 实测
            # mingw 候选挂掉静默回退 msvc，SDL 变 .lib 产物）
            args += ["-DCMAKE_RC_COMPILER=" + rc.replace("\\", "/")]
    ...
    # ---- MACOS / IOS: 需要 macOS 主机 (Xcode clang) ----
        ...
        args += ["-DCMAKE_C_COMPILER=" + cc, "-DCMAKE_CXX_COMPILER=" + cxx]
        # CI-PATCH8: MACOS 必须显式传目标架构——SDL 的 cmake/macros.cmake
        # 有自带 CPU 探测（"Detecting Target CPU Architecture"），在
        # Apple Silicon runner 上探到 ARM64 后按主机架构编译，无视交叉
        # 编译意图（macos x86 job 实测：254 个编译行无一 -arch 旗标，
        # x86_64 组合产出 arm64-only 的 libSDL3，被下游架构校验拒绝）。
        # CMAKE_OSX_ARCHITECTURES 会让 CMake 给每个编译/链接命令显式加
        # -arch，压制 SDL 的主机探测。
        if platform == "MACOS":
        ...
        # iOS/macOS 上根本不执行——传 -D 无效（iOS job 实测链接行仍缺框架）。
        # SDL_NOTIFICATION=ON 编入的 SDL_cocoanotification.m 引用
        # UNUserNotificationCenter/SecRandomCopyBytes，直接给链接器补框架。
        # CI-PATCH2: MACOS 同样需要——macos shared 实测 SDL_cocoanotification
        # 引 "_SecRandomCopyBytes"/"_UNNotificationDefaultActionIdentifier"
        # undefined（静态归档不链接所以 static 从不暴露）
...
def main():
    global _BATCH_FLAG
    args = parse_args()
    libs_wanted = parse_libs_arg(args.libs, _ALLOWED_LIBS, "sdl")
    if "sdl" not in libs_wanted:
        print("[sdl] --libs 未包含 sdl，整组跳过")
        return
    ...
    # CI-PATCH: 接受 OSX 别名——ci_build 对 macOS job 传 --platforms OSX
    # （tracy_build 等组用 OSX），本脚本平台表只认 MACOS，未知平台被忽略
    # 后整组静默跳过（macos job 实测 "(没有执行任何编译)"），SDL3 双类型
    # 全缺且连坐 imgui shared（其依赖 sdl3 产物）。归一为 MACOS。
    platforms = ["MACOS" if p == "OSX" else p for p in platforms]
