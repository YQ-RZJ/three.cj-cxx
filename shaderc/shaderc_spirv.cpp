...
namespace bgfx
{
	// 注意：shaderc_capi 作为库与 libbgfx.a 一起链接，
	// bgfx.cpp 已导出同名全局符号（g_allocator / TinyStlAllocator::static_*），
	// 这里必须用内部链接（static / struct 内联定义）避免 duplicate symbol。
	...
	static bool compile(const Options& _options, uint32_t _version, const std::string& _code, bx::WriterI* _shaderWriter, bx::WriterI* _messageWriter, bool _firstPass)
	{
		...
		if (!compiled)
		{
			...
			if (NULL != log)
			{
				...
				// bx::satSub 仅存在于新版 bx，旧版用 max(line,1)-1 等价替代
				printCode(_code.c_str(), bx::max<uint32_t>(line, 1u) - 1u, start, end, column);
