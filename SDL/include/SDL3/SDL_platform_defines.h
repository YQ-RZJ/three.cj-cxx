...
/**
 * A preprocessor macro that is only defined if compiling for OpenHarmony
 * (OHOS).
 *
 * OHOS is a Linux-compatible system but is not Linux; it also defines
 * SDL_PLATFORM_UNIX through __unix__/__unix.  Like Android we undefine the
 * generic Linux macro so SDL treats OHOS as its own platform.
 */
