...
static bool windowCondition = false;
...
bool OhosPluginManager::GetFirstNativeXComponent(std::string &id, OH_NativeXComponent **nativeXComponent)
{
    if (nativeXComponentMap.empty()) {
        *nativeXComponent = nullptr;
        return false;
    }
    auto iter = nativeXComponentMap.begin();
    id = iter->first;
    *nativeXComponent = iter->second;
    return (*nativeXComponent != nullptr);
}
