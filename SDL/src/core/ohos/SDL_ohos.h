...
extern SDL_AtomicInt bPermissionRequestPending;
extern bool bPermissionRequestResult;
extern int g_windowId;

/* Cursor support */
extern int OHOS_CreateCustomCursor(SDL_Surface *xcomponent, int hotX, int hotY);
extern bool OHOS_SetCustomCursor(int cursorID);
extern bool OHOS_SetSystemCursor(int cursorID);

/* Relative mouse support */
extern bool OHOS_SupportsRelativeMouse(void);
extern bool OHOS_SetRelativeMouseEnabled(bool enabled);
...
extern bool OHOS_NAPI_RequestPermission(const char *Permission);
...
extern void OHOS_NAPI_SetWindowStyle(bool fullscreen);
extern void OHOS_NAPI_SetOrientation(int w, int h, int resizable, const char *hint);
extern void OHOS_NAPI_ShowTextInputKeyboard(bool fullscreen);
...
extern bool OHOS_IsNodeControllerReady(void);
