...
public:
    ...
    // 取出注册表里的第一个 XComponent（用于"无 NodeController 适配层、XComponent 直连建窗"场景；
    // 该场景应用只有一个 libraryname='SDL3' 的 XComponent，取首项即可）。
    bool GetFirstNativeXComponent(std::string &id, OH_NativeXComponent **nativeXComponent);
