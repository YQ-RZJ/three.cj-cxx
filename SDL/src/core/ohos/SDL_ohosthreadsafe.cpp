...
static void OHOS_TS_SetWindowStyle(const cJSON *root)
{
    ...
    bool fullscreen = (bool)data->type;
    size_t argc = OHOS_THREADSAFE_ARG1;
    napi_value args[OHOS_THREADSAFE_ARG1] = {nullptr};
    napi_value argv[OHOS_THREADSAFE_ARG1] = {nullptr};
...
static bool OHOS_SupportsRelativeMouse(void)
{
    return true;
}

static bool OHOS_SetRelativeMouseEnabled(bool enabled)
{
    return true;
}
...
static int OHOS_RunMain(void *mainFucInfo)
{
    ...
    // argvs 已在 OHOS_NAPI_SDLAppEntry 中用 SDL_calloc 堆分配，
    // 必须用 SDL_free 释放(SDL_stack_free 在本平台是 no-op,会造成泄漏)。
    SDL_free(info->argvs);
...
bool OHOS_RunThread(OhosSDLEntryInfo *info)
{
    g_threadName++;
    std::string threadname = OHOS_THREAD_NAME + g_threadName;
    SDL_Thread *gSdlMainThread = SDL_CreateThread(OHOS_RunMain, threadname.c_str(), info);
    gSdlMainThreadList.insert(gSdlMainThread);
    return gSdlMainThread == NULL ? false : true;
}
