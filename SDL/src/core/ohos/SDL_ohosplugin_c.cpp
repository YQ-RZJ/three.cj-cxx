...
bool OHOS_GetFirstNativeXComponent(char **id, OH_NativeXComponent **nativeXComponent)
{
    *id = nullptr;
    *nativeXComponent = nullptr;
    ...
    std::string strId;
    OH_NativeXComponent *temp = nullptr;
    bool isFind = OhosPluginManager::GetInstance()->GetFirstNativeXComponent(strId, &temp);
    if (isFind) {
        *id = SDL_strdup(strId.c_str());
        *nativeXComponent = temp;
    }
    SDL_UnlockMutex(g_ohosPageMutex);
    return isFind;
