...
{
    return OHOS_WrapCursor(0, SDL_SYSTEM_CURSOR_DEFAULT);
}
...
static SDL_Cursor *OHOS_CreateCursor(SDL_Surface * xcomponent, int hot_x, int hot_y)
{
    ...
    converted = SDL_ConvertSurface(xcomponent, SDL_PIXELFORMAT_ARGB8888);
    if (!converted) {
        return NULL;
    }
...
{
    if (!empty_cursor) {
        SDL_Surface *empty_xcomponent = SDL_CreateSurface(1, 1, SDL_PIXELFORMAT_ARGB8888);
        if (empty_xcomponent) {
            ...
            SDL_DestroySurface(empty_xcomponent);
        }
    }
    return empty_cursor;
}
...
static int OHOS_ShowCursor(SDL_Cursor *cursor)
{
    ...
    if (cursor) {
        SDL_OHOSCursorData *data = (SDL_OHOSCursorData *)cursor->internal;
...
static int OHOS_SetRelativeMouseMode(bool enabled)
{
...
void OHOS_OnMouse(SDL_Window *window, OHOSWindowSize *windowsize, bool relative)
{
    int changes;
    Uint8 button;
    ...
    /* OHOS XComponent 鼠标坐标为物理像素，需除以 density 转为逻辑像素，
       因为 SDL_SendMouseMotion 内部按 window->w/h（逻辑尺寸）裁剪坐标。 */
    float density = OHOS_GetDensityPixels();
    float mx = (float)windowsize->x / density;
    float my = (float)windowsize->y / density;
    ...
    switch (windowsize->action) {
        case ACTION_DOWN:
            ...
            SDL_SendMouseMotion(0, window, 0, relative, mx, my);
            SDL_SendMouseButton(0, window, 0, button, true);
            break;
        ...
        case ACTION_UP:
            ...
            last_state = windowsize->state;
            SDL_SendMouseMotion(0, window, 0, relative, mx, my);
            SDL_SendMouseButton(0, window, 0, button, false);
            break;
