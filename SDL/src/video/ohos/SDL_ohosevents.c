...
void OHOS_PUMPEVENTS_Blocking(SDL_VideoDevice *thisDevice)
{
    ...
    SDL_VideoData *videodata = (SDL_VideoData *)thisDevice->internal;
    if (videodata->isPaused) {
        bool isContextExternal = false;
        ...
        SDL_WaitSemaphore(g_ohosResumeSem);
        {
            ...
            SDL_SendWindowEvent(ohosWindow, SDL_EVENT_WINDOW_RESTORED, 0, 0);
            ...
            if (SDL_TextInputActive(ohosWindow)) {
                OHOS_StartTextInput(thisDevice); /* Only showTextInput */
            }
        }
    } else {
        if (videodata->isPausing || SDL_TryWaitSemaphore(g_ohosPauseSem) == 0) {
            /* g_ohosPauseSem was signaled */
            if (videodata->isPausing == 0) {
                SDL_SendWindowEvent(ohosWindow, SDL_EVENT_WINDOW_MINIMIZED, 0, 0);
             ...
             * has reached the app */
            if (SDL_NumberOfEvents(SDL_EVENT_DID_ENTER_BACKGROUND) > SDL_GetSemaphoreValue(g_ohosPauseSem)) {
...
void OHOS_PUMPEVENTS_NonBlocking(SDL_VideoDevice *thisDevice)
{
    ...
    if (videodata->isPaused) {
        ...
        if (SDL_TryWaitSemaphore(g_ohosResumeSem) == 0) {
            videodata->isPaused = 0;
